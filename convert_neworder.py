...
# newOrder
dict_dir = 'app/dictionaries/newOrder'
files = ['en.ts', 'ja.ts', 'de.ts', 'fr.ts', 'id.ts', 'ko.ts', 'pt.ts', 'zh.ts', 'es.ts']

for file in files:
    filepath = os.path.join(dict_dir, file)
    lang_code = file.replace('.ts', '')
    convert_file(filepath, 'NewOrderLocale', lang_code)

print('\nAll newOrder files converted!')
